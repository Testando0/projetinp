  ...
  switch (type) {
    ...
    case 'NEW_OC':
      if (!STATE.ocs.find(o => o.id === payload.id)) {
        ...
        // FIX: operador lógico estava sem parênteses — comportamento era sempre true
        if (me && (me.cargo !== 'delegado' || payload.delegadoUser !== me.user)) {
    ...
    case 'OC_UPDATED': {
      // FIX: bloco {} necessário para const dentro de case
    ...
    case 'NEW_PUN':
      ...
      toast('Nova punição registrada para ' + payload.nome, 'w');
      break;
    ...
    case 'NEW_PONTO':
      ...
      if (me && payload.userLogin !== me.user) {
        toast(payload.nome + ' bateu ponto às ' + payload.hora, 'i');
      }
      break;
    ...
    case 'AUDIT_CLEARED':
      // FIX: case duplicado removido — o segundo nunca executava
      STATE.audit = [];
      if (activeTab === getTabIdx('audit')) renderTab(activeTab);
      break;
...
function canSeePuns()  { return me && CARGO_PERM[me.cargo] >= 1; }
function canSeePontos(){ return me && CARGO_PERM[me.cargo] >= 3; }

function getTabIdx(name) {
  if (!me) return -1;
  return tabDefs(me.cargo).findIndex(t => t.key === name);
}
...
function checkSession() {
  try {
    ...
    if (saved) {
      ...
      me = parsed;
      // Carrega cache do localStorage IMEDIATAMENTE (antes do WS conectar)
      _loadStateFromCache();
      showPanel();
    } else {
      showLogin();
...
// Carrega estado do localStorage para mostrar dados enquanto WS reconecta
function _loadStateFromCache() {
  if (typeof LSCache === 'undefined') return;
  const cache = LSCache.load();
  if (!cache) return;
  if (Array.isArray(cache.ocs))    STATE.ocs    = cache.ocs;
  if (Array.isArray(cache.puns))   STATE.puns   = cache.puns;
  if (Array.isArray(cache.pontos)) STATE.pontos = cache.pontos;
  if (Array.isArray(cache.users))  STATE.users  = cache.users;
  if (Array.isArray(cache.audit))  STATE.audit  = cache.audit;
  console.log('[Cache] Estado restaurado do localStorage:', {
    ocs: STATE.ocs.length, puns: STATE.puns.length, pontos: STATE.pontos.length
  });
}
...
async function login() {
  ...
  // FIX: desabilitar botão durante requisição — evita duplo clique
  ...
  try {
    ...
    me = res.user;
    // FIX: garantir que 'pass' nunca vai para sessionStorage
    const { pass: _p, ...meSafe } = me;
    me = meSafe;
    sessionStorage.setItem('gmpol_session', JSON.stringify(me));
    // FIX: resetar activeTab ao logar — evita aba inválida de sessão anterior
    activeTab = 0;
    showPanel();
    ...
    // FIX: sempre reabilitar botão, mesmo em caso de erro
    if (btn) { btn.disabled = false; btn.textContent = '▸ AUTENTICAR'; }
  }
}

// FIX: botão de mostrar/ocultar senha
...
function showLogin() {
  ...
  // FIX: focar campo usuário automaticamente
  setTimeout(() => { const el = document.getElementById('l-user'); if (el) el.focus(); }, 80);
}
...
function showPanel() {
  ...
  // FIX: sempre resetar aba para 0 ao mostrar painel
  activeTab = 0;
  buildTabs();
  renderTab(0);
  updateNotif();
...
function buildTabs() {
  ...
  // Build mobile drawer simultaneously
  buildMobileDrawer();
}
...
function buildMobileDrawer() {
  ...
  if (!drawer) return;
  drawer.innerHTML = defs.map((t, i) =>
    '<div class="nav-drawer-item' + (i === activeTab ? ' active' : '') + '" onclick="switchTab(' + i + ');closeNavDrawer();">' +
    t.label +
    (t.notif ? '<span class="tab-n-badge" id="tnd-' + i + '" style="display:none">0</span>' : '') +
    '</div>'
  ).join('');
}
...
function renderTab(idx) {
  ...
  const views = {
    home: vInicio, ocs: vOcAdmin, hist: vHistorico, registrar: vRegistrar,
    myocs: vOcDelegado, puns: vPunicoes, users: vUsuarios, pontos: vPontos, audit: vAuditoria
  };

  document.getElementById('content').innerHTML = (views[def.key] || vInicio)();

  // FIX: relógio gerenciado aqui, sem override frágil de função global
  if (def.key === 'pontos') { setTimeout(startClock, 50); }
...
function updateNotif() {
  ...
  const ocTabIdx = tabDefs(me.cargo).findIndex(t => t.key === 'ocs');
...
function vInicio() {
  ...
  return `
    ...
    ${showStats ? `<div class="g3" style="grid-template-columns:repeat(4,1fr);">
    ...
    <div class="card" style="margin-top:20px;">
      ...
      ${STATE.audit.slice(0,5).map(l => `<div class="log-entry" style="padding:8px 0;border-bottom:1px solid var(--border);">
    ...
    <div class="card">
      ...
      <div class="g2">
        <div class="fg"><label>Tipo de Registro</label>
          <select id="oc-tipo"><option value="Ocorrência">Ocorrência</option><option value="Denúncia">Denúncia</option></select>
        </div>
        ...
        <div class="fg"><label>Cargo do Envolvido</label>
          <select id="oc-cargo">
            <option>Guarda Municipal</option><option>Agente de Polícia</option><option>Policiamento Tático</option>
            <option>Escrivão de Polícia</option><option>Delegado de Polícia</option><option>Chefe de Polícia</option>
...
async function registrarOc() {
  ...
  const newOc = {
    id: (tipo === 'Denúncia' ? 'DN' : 'OC') + '-' + Date.now(),
    tipo, autor: me.nome, autorUser: me.user, autorCargo: me.cargo,
    delegado: me.nome, delegadoUser: me.user, nome, cargo,
    depoimento: dep, status: 'pendente', resposta: '', ts: Date.now()
  };
  try {
...
function vOcAdmin() {
  ...
  return '<div class="stitle">▸ REGISTROS PENDENTES' + (isMaster ? ' <span style="font-size:.65rem;color:var(--text-dim);">[MASTER]</span>' : '') + '</div>' +
...
function vHistorico() {
  const ocs = STATE.ocs.filter(o => o.status !== 'pendente').reverse();
  const isMaster = me.cargo === 'master';
  return `<div class="stitle">▸ HISTÓRICO DE OCORRÊNCIAS</div>
    ...
    <div id="hist-list">${ocs.length ? ocs.map(o => ocCard(o, false, isMaster)).join('') : empty('📂', 'Nenhuma ocorrência processada.')}</div>`;
}
...
function filtrarHist(status) {
  ...
  if (el) el.innerHTML = ocs.length ? ocs.map(o => ocCard(o, false, isMaster)).join('') : empty('📂', 'Nenhuma ocorrência nesta categoria.');
}
...
async function decidir(id) {
  ...
  if (!oc) return;
  try {
    await API.updateOc(id, { ...oc, status: acao, resposta: me.nome + ': ' + motivo, decididoPor: me.user, decididoEm: Date.now() });
    toast('Ocorrência ' + (acao === 'aceita' ? 'aceita ✅' : 'recusada ❌') + '!', acao === 'aceita' ? 's' : 'd');
    updateNotif();
  } catch (e) { toast(e.message || 'Erro ao processar decisão.', 'd'); }
}
...
async function salvarEdicaoOc() {
  ...
  try {
    await API.updateOc(_editOcId, {
      ...
      editadoPor: me.user, editadoEm: Date.now()
    });
    closeModal('m-edit-oc');
    toast('Ocorrência editada com sucesso!', 's');
  } catch (e) { toast(e.message || 'Erro ao editar ocorrência.', 'd'); }
}

function confirmarDeleteOc(id) {
  const oc = STATE.ocs.find(o => o.id === id);
  if (!oc) return;
  document.getElementById('del-info').innerHTML = 'Tem certeza que deseja <b>EXCLUIR PERMANENTEMENTE</b> a ocorrência <b>' + id + '</b> registrada por ' + oc.delegado + ' sobre ' + oc.nome + '? Esta ação não pode ser desfeita.';
...
async function deleteOc(id) {
  try { await API.deleteOc(id, me.user); toast('Ocorrência excluída.', 'w'); }
  catch (e) { toast(e.message || 'Erro ao excluir ocorrência.', 'd'); }
}

async function cancelarOc(id) {
  const oc = STATE.ocs.find(o => o.id === id);
  if (!oc) return;
  try { await API.updateOc(id, { ...oc, status: 'cancelada', canceladoPor: me.user, canceladoEm: Date.now() }); toast('Ocorrência cancelada.', 'w'); }
  catch (e) { toast(e.message || 'Erro ao cancelar ocorrência.', 'd'); }
}
    ...
    return `<tr>
      <td>
        <div style="display:flex;align-items:center;gap:10px;">
          ...
          <div><div style="font-weight:600;">${u.nome}</div>
          <div style="font-family:'Share Tech Mono',monospace;font-size:.6rem;color:var(--text-dim);">@${u.user}</div></div>
        </div>
      </td>
      <td>${isMaster && !isMe && !isM
        ? '<select class="cargo-select" onchange="alterarCargo(\'' + u.user + '\', this.value)">' +
          Object.entries(CARGO_LABEL).filter(([k]) => k !== 'master').map(([k,v]) => '<option value="' + k + '" ' + (u.cargo===k?'selected':'') + '>' + v + '</option>').join('') +
          '</select>'
      ...
      <td>
        <div style="display:flex;gap:6px;flex-wrap:wrap;">
          ...
          ${canEdit && isMaster ? '<button class="btn btn-xs ' + (u.ativo?'btn-danger':'btn-success') + '" onclick="toggleStatus(\'' + u.user + '\',' + (!u.ativo) + ')">' + (u.ativo?'🚫 Desativar':'✅ Ativar') + '</button><button class="btn btn-danger btn-xs" onclick="confirmarDeleteUser(\'' + u.user + '\',\'' + u.nome + '\')">🗑 EXCLUIR</button>' : ''}
          ${canEdit && !isMaster ? '<button class="btn btn-danger btn-xs" onclick="confirmarDeleteUser(\'' + u.user + '\',\'' + u.nome + '\')">✘ REMOVER</button>' : ''}
        </div>
      </td>
    </tr>`;
  }).join('');
...
async function criarUsuario() {
  ...
  } catch (e) { toast(e.message || 'Erro ao criar usuário.', 'd'); }
}
...
async function confirmarResetSenha() {
  ...
  try { await API.resetSenha(username, nova, me.nome); toast('Senha redefinida com sucesso!','s'); closeModal('m-reset-senha'); }
  catch (e) { toast(e.message || 'Erro ao redefinir senha.','d'); }
}

function confirmarDeleteUser(username, nome) {
  document.getElementById('del-info').innerHTML = 'Tem certeza que deseja <b>EXCLUIR</b> o usuário <b>' + nome + '</b> (@' + username + ')? Esta ação remove o acesso permanentemente.';
  document.getElementById('del-confirm-btn').onclick = () => { closeModal('m-confirm-del'); deleteUser(username); };
  openModal('m-confirm-del');
}

async function deleteUser(username) {
  try { await API.deleteUser(username, me.nome); toast('Usuário excluído.','w'); }
  catch (e) { toast(e.message || 'Erro ao excluir usuário.','d'); }
}

async function toggleStatus(username, ativo) {
  try { await API.toggleUserStatus(username, ativo, me.nome); toast('Conta ' + (ativo?'ativada':'desativada') + ' com sucesso.', ativo?'s':'w'); }
  catch (e) { toast(e.message || 'Erro ao alterar status.','d'); }
}

async function alterarCargo(username, cargo) {
  try { await API.updateUserCargo(username, cargo, me.nome); toast('Cargo alterado com sucesso.','s'); }
  catch (e) { toast(e.message || 'Erro ao alterar cargo.','d'); }
}
...
function vPunicoes() {
  ...
  const form = canEdit ? `<div class="card" style="margin-bottom:22px;">
    ...
    <div class="g2">
      ...
      <div class="fg"><label>Nível</label><select id="pn-nivel"><option>Leve</option><option>Médio</option><option>Grave</option></select></div>
  ...
  const rows = [...puns].reverse().map((p, ri) => {
    ...
    return '<tr><td style="font-weight:600;">' + p.nome + '</td><td style="color:var(--text-mid);font-size:.88rem;">' + p.motivo + '</td><td><span class="status-chip ' + nc(p.nivel) + '">' + p.nivel + '</span></td><td style="font-family:\'Share Tech Mono\',monospace;font-size:.62rem;color:var(--text-dim);">' + p.autor + '</td><td style="font-family:\'Share Tech Mono\',monospace;font-size:.6rem;color:var(--text-dim);">' + new Date(p.ts).toLocaleDateString('pt-BR') + '</td>' + (canEdit||isMaster ? '<td><button class="btn btn-danger btn-xs" onclick="delPun(' + realIdx + ')">✘</button></td>' : '<td></td>') + '</tr>';
  }).join('');
  return '<div class="stitle">▸ QUADRO DE PUNIÇÕES</div>' + form +
    '<div class="card c-none" style="padding:0;overflow:hidden;"><table class="tbl"><thead><tr><th>NOME</th><th>MOTIVO</th><th>NÍVEL</th><th>REGISTRADO POR</th><th>DATA</th><th></th></tr></thead><tbody>' +
    (rows || '<tr><td colspan="6" style="text-align:center;padding:30px;font-family:\'Share Tech Mono\',monospace;font-size:.68rem;color:var(--text-dim);">Nenhuma punição registrada.</td></tr>') +
    '</tbody></table></div>';
}
...
async function addPun() {
  ...
  } catch (e) { toast(e.message || 'Erro ao registrar punição.','d'); }
}

async function delPun(idx) {
  try {
    const pun = STATE.puns[idx];
    if (pun && pun.id) await API.deletePunById(pun.id, me.nome);
    else await API.deletePun(idx, me.nome);
    toast('Punição removida.','w');
  } catch (e) { toast(e.message || 'Erro ao remover punição.','d'); }
}
...
function vPontos() {
  ...
  const jaHoje       = meusPontos.some(p => new Date(p.ts).toDateString() === hoje);
  const porUsuario   = {};
  STATE.pontos.forEach(p => { if (!porUsuario[p.userLogin]) porUsuario[p.userLogin] = []; porUsuario[p.userLogin].push(p); });
  const pontosHoje = STATE.pontos.filter(p => new Date(p.ts).toDateString() === hoje);

  const minhaTabela = meusPontos.length ? '<table class="tbl"><thead><tr><th>DATA</th><th>HORA</th><th>DIA DA SEMANA</th></tr></thead><tbody>' +
    [...meusPontos].reverse().slice(0,20).map(p => { const d=new Date(p.ts); return '<tr><td style="font-family:\'Share Tech Mono\',monospace;">' + d.toLocaleDateString('pt-BR') + '</td><td style="font-family:\'Share Tech Mono\',monospace;color:var(--accent);font-weight:700;">' + p.hora + '</td><td style="font-family:\'Share Tech Mono\',monospace;font-size:.65rem;color:var(--text-dim);">' + d.toLocaleDateString('pt-BR',{weekday:'long'}).toUpperCase() + '</td></tr>'; }).join('') +
    '</tbody></table>' : '<p style="color:var(--text-dim);font-family:\'Share Tech Mono\',monospace;font-size:.68rem;">Nenhum ponto registrado.</p>';
  ...
  return `<div class="stitle">▸ BATER PONTO</div>
    <div class="card" style="margin-bottom:20px;text-align:center;">
      ...
      ${jaHoje ? '<div style="display:inline-block;padding:10px 24px;background:rgba(var(--accent3-rgb),.1);border:1px solid var(--accent3);border-radius:4px;font-family:\'Share Tech Mono\',monospace;font-size:.7rem;color:var(--accent3);">✅ PONTO REGISTRADO HOJE</div><div style="margin-top:10px;font-size:.78rem;color:var(--text-dim);">' + meusPontos.filter(p=>new Date(p.ts).toDateString()===hoje).map(p=>'Às ' + p.hora).join(' — ') + '</div>' :
    ...
    <div class="card" style="margin-bottom:20px;">
      ...
      ${pontosHoje.length ? '<table class="tbl"><thead><tr><th>AGENTE</th><th>HORA</th><th>CARGO</th></tr></thead><tbody>' +
        pontosHoje.map(p => '<tr><td><b>' + p.nome + '</b></td><td style="font-family:\'Share Tech Mono\',monospace;color:var(--accent);font-weight:700;">' + p.hora + '</td><td><span class="cargo-badge ' + (CARGO_BADGE_CLASS[p.cargo]||'') + '" style="font-size:.55rem;">' + (CARGO_LABEL[p.cargo]||p.cargo) + '</span></td></tr>').join('') +
        '</tbody></table>' : '<p style="color:var(--text-dim);font-family:\'Share Tech Mono\',monospace;font-size:.68rem;">Nenhum agente bateu ponto hoje.</p>'}
    </div>
    <div class="card">
      ...
      ${Object.keys(porUsuario).length ? Object.entries(porUsuario).map(([login,pts]) => {
        ...
        const nome = u ? u.nome : login; const cargo = u ? u.cargo : '';
        return '<div class="ponto-agente-block"><div class="ponto-agente-header" onclick="togglePontoAgente(\'pa-' + login + '\')"><div><div class="u-avatar" style="display:inline-flex;width:28px;height:28px;font-size:.7rem;">' + nome.charAt(0) + '</div><b style="margin-left:8px;">' + nome + '</b><span class="cargo-badge ' + (CARGO_BADGE_CLASS[cargo]||'') + '" style="font-size:.5rem;margin-left:8px;">' + (CARGO_LABEL[cargo]||cargo) + '</span></div><span style="font-family:\'Share Tech Mono\',monospace;font-size:.65rem;color:var(--text-dim);">' + pts.length + ' registro(s) ▾</span></div><div id="pa-' + login + '" class="ponto-agente-body" style="display:none;"><table class="tbl"><thead><tr><th>DATA</th><th>HORA</th><th>DIA</th></tr></thead><tbody>' +
          [...pts].reverse().map(p => { const d=new Date(p.ts); return '<tr><td style="font-family:\'Share Tech Mono\',monospace;">' + d.toLocaleDateString('pt-BR') + '</td><td style="font-family:\'Share Tech Mono\',monospace;color:var(--accent);font-weight:700;">' + p.hora + '</td><td style="font-family:\'Share Tech Mono\',monospace;font-size:.65rem;color:var(--text-dim);">' + d.toLocaleDateString('pt-BR',{weekday:'long'}).toUpperCase() + '</td></tr>'; }).join('') +
          '</tbody></table></div></div>';
...
function startClock() {
  ...
  _clockInterval = setInterval(() => {
    ...
    // FIX: auto-parar se elemento sumiu (troca de aba sem limpar)
...
async function baterPonto() {
  const now   = new Date();
  const ponto = {
    userLogin: me.user, nome: me.nome, cargo: me.cargo,
    hora: now.toLocaleTimeString('pt-BR', { hour:'2-digit', minute:'2-digit', second:'2-digit' }),
    data: now.toLocaleDateString('pt-BR'), ts: now.getTime()
  };
  try { await API.createPonto(ponto); toast('✅ Ponto registrado às ' + ponto.hora + '!', 's'); renderTab(activeTab); }
  catch (e) { toast(e.message || 'Erro ao bater ponto.', 'd'); }
}
...
function vAuditoria() {
  ...
  const items = logs.map(l =>
    '<div class="log-entry"><div class="log-time">' + new Date(l.ts).toLocaleString('pt-BR',{day:'2-digit',month:'2-digit',hour:'2-digit',minute:'2-digit'}) + '</div><div class="log-icon">' + (l.icon||'📋') + '</div><div class="log-txt">' + l.msg + '</div></div>'
  ).join('');
  return '<div class="stitle">▸ AUDITORIA DO SISTEMA</div><div style="display:flex;justify-content:flex-end;margin-bottom:12px;"><button class="btn btn-danger btn-sm" onclick="limparAuditoria()">🗑 LIMPAR LOG</button></div><div class="card c-none" style="max-height:580px;overflow-y:auto;">' + items + '</div><div style="margin-top:10px;" class="hint">Últimos <span>' + logs.length + '</span> evento(s) registrado(s).</div>';
}

async function limparAuditoria() {
  if (!confirm('Limpar todo o histórico de auditoria?')) return;
  try { await API.clearAudit(); toast('Auditoria limpa.','w'); }
  catch (e) { toast(e.message || 'Erro ao limpar auditoria.','d'); }
}
