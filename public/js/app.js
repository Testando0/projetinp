...
async function login() {
  ...
  try {
    ...
    me = res.user;
    const { pass: _p, ...meSafe } = me; me = meSafe; sessionStorage.setItem('gmpol_session', JSON.stringify(me));
    showPanel();
...
async function delPun(idx) {
  try {
    ...
    if (pun && pun.id) {
      await API.deletePunById(pun.id, me.nome);
    } else {
...
async function limparAuditoria() {
  ...
  try {
    ...
    // STATE.audit será zerado via broadcast AUDIT_CLEARED
