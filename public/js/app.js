...
async function checkSession(){
  try{
    ...
    if(saved){
      ...
      me=parsed; _loadStateFromCache();
      try{const bc=await API.checkBan(me.user);if(bc.banned){showBanScreen(bc);return;}}catch(_){}
      showPanel();
    } else showLogin();
...
async function login(){
  ...
  try{
    ...
    me=res.user; const{pass:_p,...meSafe}=me; me=meSafe;
...
function toggleSenhaVisivel(){
  ...
  if(!i)return;
  if(i.type==='password'){i.type='text';if(b)b.textContent='🙈';}else{i.type='password';if(b)b.textContent='👁';}
}
...
function vPontos(){
  ...
  return `<div class="stitle">▸ BATER PONTO</div>
    <div class="card" style="margin-bottom:20px;text-align:center;">
      ...
      ${jaHoje?'<div style="display:inline-block;padding:10px 24px;background:rgba(var(--accent3-rgb),.1);border:1px solid var(--accent3);border-radius:4px;font-family:\'Share Tech Mono\',monospace;font-size:.7rem;color:var(--accent3);">✅ PONTO REGISTRADO HOJE</div>':'<button class="btn btn-primary" style="font-size:.85rem;padding:12px 32px;" onclick="baterPonto()">▸ BATER PONTO AGORA</button>'}
    </div>
    <div class="card" style="margin-bottom:20px;"><div style="font-family:'Orbitron',sans-serif;font-size:.68rem;color:var(--accent);letter-spacing:.12em;margin-bottom:12px;">▸ MEUS REGISTROS</div>${minhaTab}</div>
    ${isSuperv?`<div class="card" style="margin-bottom:20px;"><div style="font-family:'Orbitron',sans-serif;font-size:.68rem;color:var(--accent);letter-spacing:.12em;margin-bottom:12px;">▸ PONTOS HOJE</div>${hoje2.length?'<table class="tbl"><thead><tr><th>AGENTE</th><th>HORA</th><th>CARGO</th></tr></thead><tbody>'+hoje2.map(p=>'<tr><td><b>'+p.nome+'</b></td><td style="font-family:\'Share Tech Mono\',monospace;color:var(--accent);font-weight:700;">'+p.hora+'</td><td><span class="cargo-badge '+(CARGO_BADGE_CLASS[p.cargo]||'')+'" style="font-size:.55rem;">'+(CARGO_LABEL[p.cargo]||p.cargo)+'</span></td></tr>').join('')+'</tbody></table>':'<p style="color:var(--text-dim);font-family:\'Share Tech Mono\',monospace;font-size:.68rem;">Nenhum hoje.</p>'}</div><div class="card"><div style="font-family:\'Orbitron\',sans-serif;font-size:.68rem;color:var(--accent);letter-spacing:.12em;margin-bottom:12px;">▸ HISTÓRICO POR AGENTE</div>`+Object.entries(porUser).map(([login,pts])=>{const u=STATE.users.find(u=>u.user===login);const nm=u?u.nome:login;const cg=u?u.cargo:'';return '<div class="ponto-agente-block"><div class="ponto-agente-header" onclick="togglePontoAgente(\'pa-'+login+'\')"><div><div class="u-avatar" style="display:inline-flex;width:28px;height:28px;font-size:.7rem;">'+nm.charAt(0)+'</div><b style="margin-left:8px;">'+nm+'</b><span class="cargo-badge '+(CARGO_BADGE_CLASS[cg]||'')+'" style="font-size:.5rem;margin-left:8px;">'+(CARGO_LABEL[cg]||cg)+'</span></div><span style="font-family:\'Share Tech Mono\',monospace;font-size:.65rem;color:var(--text-dim);">'+pts.length+' reg. ▾</span></div><div id="pa-'+login+'" style="display:none;"><table class="tbl"><thead><tr><th>DATA</th><th>HORA</th><th>DIA</th></tr></thead><tbody>'+[...pts].reverse().map(p=>{const d=new Date(p.ts);return '<tr><td style="font-family:\'Share Tech Mono\',monospace;">'+d.toLocaleDateString('pt-BR')+'</td><td style="font-family:\'Share Tech Mono\',monospace;color:var(--accent);font-weight:700;">'+p.hora+'</td><td style="font-family:\'Share Tech Mono\',monospace;font-size:.65rem;color:var(--text-dim);">'+d.toLocaleDateString('pt-BR',{weekday:'long'}).toUpperCase()+'</td></tr>';}).join('')+'</tbody></table></div></div>';}).join('')+'</div>`:''}`;}
...
async function alterarSenhaPropria(){
  ...
  if(!at||!nv||!cf){toast('Preencha todos.','d');return;}if(nv.length<6){toast('Mínimo 6 chars.','w');return;}if(nv!==cf){toast('Senhas diferentes.','d');return;}
  try{await API.login(me.user,at);}catch(e){toast('Senha atual incorreta.','d');return;}
  try{await API.resetSenha(me.user,nv,me.nome);toast('Senha alterada!','s');closeModal('m-senha');['s-atual','s-nova','s-conf'].forEach(id=>document.getElementById(id).value='');}
  catch(e){toast(e.message||'Erro.','d');}
}
