...
function vPontos(){
  ...
  // ═══ DETECTAR ESTADO ATUAL DO TURNO ═══
  const emPausa = lastPonto && lastPonto.type === 'pausa_inicio';
  const emTurno = lastPonto && (lastPonto.type === 'entrada' || lastPonto.type === 'pausa_retomar');
  ...
  if(!lastPonto || lastPonto.type==='saida' || lastPonto.type==='folga'){
    ...
    botaoPonto='<div style="display:flex;gap:10px;flex-wrap:wrap;justify-content:center;">'
      ...
      +'</div>'
      +'<div style="margin-top:10px;'+FM+'font-size:.68rem;color:var(--accent);">Turno ativo desde '+ultimaEntrada.hora+'</div>';
  }
  var supervHtml='';
  if(isSuperv){
